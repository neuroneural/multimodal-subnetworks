...
Three "seed sources" on one class, each matching a spot in the training code:
  1. attr_seed   - a plain attribute set in __init__ from a value passed in.
  ...
  3. module_seed - a method that reads the module-level TOP_SEED.
                   (This is the old sampler reading the top-of-file `SEED`.)
...
def worker(worker_id, exp, q):
    ...
    q.put((
        worker_id, os.getpid(),
        exp.attr_seed,      # frozen at creation -> same as parent
...
def main():
    ...
    print(f"parent: created exp with attr_seed={exp.attr_seed}")
    print(f"parent: exp.prop_seed read#1={exp.prop_seed} read#2={exp.prop_seed}")
    print("        ^ two reads differ -> the @property is recomputed each time, NOT fixed at creation")
    print(f"parent: exp.module_seed()={exp.module_seed()} (== TOP_SEED)\n")
    ...
    for wid, pid, attr, p1, p2, mod in rows:
        print(f"worker {wid} (pid {pid}): attr_seed={attr:<6} "
              f"prop#1={p1:<11} prop#2={p2:<11} module_seed={mod}")

    attr_vals = {r[2] for r in rows}
    prop_vals = {r[3] for r in rows} | {r[4] for r in rows}
    mod_vals = {r[5] for r in rows}
    print()
    print(f"attr_seed   (attribute, passed in) : {sorted(attr_vals)}  "
          f"-> SAME everywhere (pickled with the object)")
    print(f"prop_seed   (@property, os.urandom): {len(prop_vals)} distinct values over {2*n} reads  "
