...
#SBATCH --gres=gpu:A100:2       # OVERRIDE per run: sbatch --gres=gpu:A100:1 05_data.sh
#SBATCH -J seed05               # Job name
#SBATCH -D .                    # start in the dir you submit from
#SBATCH --output=seed05-%j.out  # .out lands right where you run sbatch
#SBATCH -A psy53c17             # allocation/project name

# ---------------------------------------------------------------------------
# Stage 5: what data each rank receives per candidate seed (05_data.py).
# Submit from this (random_seed/) directory, picking the GPU count on the CLI:
#
#     sbatch --gres=gpu:A100:1 05_data.sh
#     sbatch --gres=gpu:A100:2 05_data.sh
#     sbatch --gres=gpu:A100:4 05_data.sh
# ---------------------------------------------------------------------------
